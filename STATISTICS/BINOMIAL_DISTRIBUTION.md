...
In the slides, each coin toss has two possible outcomes: heads or tails. We choose one outcome to call the **success**.

For example, if $\text{Success} = \text{Heads}$, then:

$$
p = P(\text{Heads}) \qquad 1 - p = P(\text{Tails})
$$
...
A binomial distribution is determined by two values, $n$ and $p$:
...
$X$ is the **random variable**. For example:
...
$x$ is one particular possible value of $X$. For example, $x = 2$ means exactly 2 heads.
...
The expression $\binom{n}{x}$ is called the **binomial coefficient**. It tells you:
...
The slides also describe this as "$n$ choose $x$."
...
There are **10 different ways to get exactly 2 heads in 5 tosses**. This is the example developed on pages 5–6 of the slides.

---

# 11. Why the binomial coefficient is necessary

Suppose you want exactly 2 heads in 5 tosses. One possible sequence is $HHTTT$, but you could also get $HTHTT$, or $HTTHT$, or many other arrangements.

The probability formula needs to account for **all possible orders** that produce 2 heads. That is exactly what $\binom{5}{2}$ does, and the slides show that there are 10 such arrangements.
...
The expression $p^x$ represents the probability contribution from the **successes**.

If $x = 2$, then $p^x = p^2$, because there are 2 successes. The slides identify $p^x$ as the portion corresponding to the heads/successes.

---

# 13. What $(1-p)^{n-x}$ represents

The probability of failure is $1-p$. If there are $n$ trials and $x$ successes, then the number of failures is:

$$
\boxed{n - x}
$$

Therefore $(1-p)^{n-x}$ represents the failure portion.

For example, $n = 5$ and $x = 2$ gives $n - x = 5 - 2 = 3$, so:
...
It helps to think of $P(X = x) = \binom{n}{x} p^x (1-p)^{n-x}$ as three pieces:
...
Substitute into $P(X = x) = \binom{n}{x} p^x (1-p)^{n-x}$:
...
For a fair coin, $p = 0.5$. The slides show the 5-flip distribution:
...
Notice that $P(X=0) = P(X=5)$, $P(X=1) = P(X=4)$, and $P(X=2) = P(X=3)$. So when $p = 0.5$, the graph in the slides is symmetric.
...
The slides compare $n = 5$ with $p = 0.5$ against $n = 5$ with $p = 0.3$.

When $p = 0.3$, successes are less likely, so probability becomes concentrated around **smaller numbers of successes**. The slide gives approximately:
...
$$
\boxed{p \text{ controls how likely success is}}
$$
...
Coins are just an easy example. The slides also use dice.
...
Rolling a die repeatedly can therefore be treated like repeated success/failure trials, where rolling a 1 is a success and anything else is a failure. The slides describe this as treating the die like a **biased coin** for the chosen event.
...
A binomial distribution counts the total successes across **multiple trials**. For example, $X = \text{number of heads in 5 flips}$ could be $0, 1, 2, 3, 4, 5$.

The slides end by showing the related one-trial success/failure setup — a single trial like this is called a **Bernoulli** trial, and the binomial distribution is what you get by counting successes across $n$ of them. The important binomial idea is:

$$
\boxed{\text{Binomial} = \text{number of successes across } n \text{ trials}}
$$
...
$$
\boxed{X = \text{random variable counting successes}}
$$

$$
\boxed{x = \text{particular number of successes}}
$$

Example: $X = \text{number of heads in 5 flips}$, while $x = 2$ means exactly 2 heads.
...
It tells you the number of ways to arrange $x$ successes among $n$ trials.

## Successes and failures

$p^x$ represents the $x$ successes, and $(1-p)^{n-x}$ represents the $n - x$ failures.
