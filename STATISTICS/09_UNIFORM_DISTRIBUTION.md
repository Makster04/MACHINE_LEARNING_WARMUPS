...
![Three uniform rectangles of equal area, the narrowest reaching a density of 2](../figures/09-height-vs-width-dark.png)
...
![The 0 to 15 minute rectangle with the 5 to 10 window shaded as one third of the area](../figures/09-rectangle-and-probability-dark.png)
...
![A flat uniform rectangle beside a bell-shaped normal curve](../figures/09-uniform-vs-normal-dark.png)
...
![A flat uniform PDF above its straight-line CDF, aligned on the same axis](../figures/09-flat-pdf-linear-cdf-dark.png)
...
The same trick works for a distribution with a handful of separate outcomes, and it is even easier to see. Suppose a colour is Green with probability $0.3$, Blue with $0.5$, and Orange with $0.2$.

Lay those probabilities end to end along $[0,1]$, draw a single $u\sim U(0,1)$, and see which slice it lands in:
...
![A PMF laid out as slices of the unit interval, with a uniform draw selecting one](../figures/09-discrete-sampling-dark.png)

A draw of $u=0.62$ lands in the middle slice, so the outcome is Blue.

Each slice has width equal to its own probability, so it gets selected exactly that often — Blue occupies half the interval and therefore comes up half the time.

## The cut points are the CDF

Look at where the slices break: $0.3$ and $0.8$. Those are not arbitrary — they are the accumulated probabilities, which is to say the **CDF values** from 06:

$$
F(\text{Green}) = 0.3 \qquad F(\text{Blue}) = 0.8 \qquad F(\text{Orange}) = 1.0
$$

So the discrete case is the same procedure as the continuous one, just with a step-function CDF instead of a smooth one:

$$
\boxed{\text{Both versions are: draw } u \sim U(0,1), \text{ then invert the CDF.}}
$$

![Uniform draws read across to a CDF and dropped down to give samples](../figures/09-inverse-transform-dark.png)
